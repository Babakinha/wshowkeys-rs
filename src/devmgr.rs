...
fn recv_msg(sock: i32, fd: *mut i32, buf: *mut c_void, buf_len: size_t) -> ssize_t {
    ...
    if fd != null_mut() {
        ...
        msghdr.msg_controllen = control.len() * size_of::<char>();
    }
    ...
    if cmsg != null_mut() {
        //memcpy(fd_out, CMSG_DATA(cmsg), size_of<*fd_out>())
    }else {
        //*fd_out = -1;
    }
...
pub fn send_msg(sock: i32, fd: i32, buf: *mut c_void, buf_len: size_t) {
    ...
    let mut msghdr: msghdr = msghdr {
        ...
        msg_iov: &mut iovec,
        msg_iovlen: 1,
        msg_control: null_mut() as *mut c_void,
        msg_controllen: 0,
        msg_flags: 0
    };
